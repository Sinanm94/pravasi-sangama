 ...
 */
export async function countExpectedGuests(): Promise<number> {
  const { rows } = await query<{ count: string }>(
 ...
 */
export async function countAdmittedToday(timezone: string): Promise<number> {
  const { rows } = await query<{ count: string }>(
    `SELECT COUNT(*)::TEXT AS count
       FROM scan_logs
...
export async function countActiveGates(withinMinutes: number): Promise<number> {
  const { rows } = await query<{ count: string }>(
    `SELECT COUNT(DISTINCT unit_id)::TEXT AS count
       FROM scan_logs
...
export async function ticketTypeBreakdown(): Promise<TicketTypeRow[]> {
  const { rows } = await query<TicketTypeRow>(
    `SELECT ticket_type,
...
export async function divisionPerformance(): Promise<DivisionRow[]> {
  const { rows } = await query<DivisionRow>(
...
export async function recentScans(limit: number): Promise<RecentScanRow[]> {
  const { rows } = await query<RecentScanRow>(
