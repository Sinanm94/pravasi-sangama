...
/* =================================================================== */
export async function unitLogin(input, ctx) {
    ...
    if (units.length > 1) {
        throw new AppError(409, 'AMBIGUOUS_UNIT_CODE', 'This unit code exists in more than one division. Include division_code.', { divisions: units.map((u) => u.division_code) });
    }
    const unit = units[0];
    if (!unit.is_active) {
        await verifyAgainstDummy(input.pin);
...
/* =================================================================== */
export async function agentLogin(input, pending, rawToken, ctx) {
...
export async function describeSession(claims) {
    ...
    return {
        ...
        agent: claims.role === 'AGENT' && ctx.agent_id
            ? {
                id: ctx.agent_id,
                name: ctx.agent_name ?? '',
...
/* =================================================================== */
export async function logout(sessionId, ctx) {
