...
async function seed() {
    ...
    await withTransaction(async (client) => {
        ...
        for (const su of SUPERUSERS) {
            const { rows } = await client.query(`INSERT INTO superusers (username, email, password_hash, name)
              VALUES ($1, $2, $3, $4)
         ON CONFLICT (username) DO UPDATE
              SET email         = EXCLUDED.email,
                  ...
                  name          = EXCLUDED.name,
                  is_active     = TRUE
           RETURNING id`, [su.username, su.email, superuserHash, su.name]);
            if (!superuserId)
        ...
        await client.query(`UPDATE superusers SET is_active = FALSE WHERE username = 'superadmin'`);
        /* --- division ------------------------------------------------- */
        ...
        for (const agent of AGENTS) {
         ...
         ON CONFLICT (mobile_number) DO UPDATE
              SET unit_id         = EXCLUDED.unit_id,
                  ...
                  is_active       = TRUE`, [unitId, agent.mobile_number, agent.name, agent.email, pinHash]);
        }
        /* --- gates (spec §2, Option A) -------------------------------- */
        for (const gate of GATES) {
            const pinHash = await hashSecret(gate.pin);
            await client.query(`INSERT INTO gates (gate_code, name, division_id, pin_hash)
              VALUES ($1, $2, $3, $4)
         ON CONFLICT (gate_code) DO UPDATE
              SET name        = EXCLUDED.name,
                  division_id = EXCLUDED.division_id,
                  pin_hash    = EXCLUDED.pin_hash,
                  is_active   = TRUE`, [gate.gate_code, gate.name, divisionId, pinHash]);
        }
