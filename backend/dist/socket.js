...
import { LIVE_EVENTS, LIVE_NAMESPACE, SESSION_COOKIE_NAME, } from '@pravasi/shared';
...
export function initSocket(server) {
    ...
    live.on('connection', (socket) => {
        ...
        console.log(`[socket] superuser ${socket.data.superuserId} connected (${live?.sockets.size ?? 0} online)`);
 ...
 */
export function broadcastScanAlert(event) {
    if (!live)
        return;
    if (event.alertType === 'DUPLICATE' ||
