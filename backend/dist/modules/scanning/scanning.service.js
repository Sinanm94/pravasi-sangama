import { SCAN_REASON_TO_RESULT, SCAN_REASON_TO_STATUS, TICKET_TYPE_LABELS, } from '@pravasi/shared';
...
/* ================================================================== */
async function resolveScan(client, args) {
    const qrHash = hashQrPayload(args.payload);
    const logIt = (reason, qrCodeId, ticketId) => repo.insertScanLog(client, {
    ...
    const summary = reason === 'LOCATION_INFO' || reason === 'ALREADY_SCANNED'
        ? await repo.ticketSummary(client, found.ticket_id)
        : null;
    await logIt(reason, found.id, found.ticket_id);
    const priorGate = reason === 'ALREADY_SCANNED'
    ...
    return respond({
        ...
        priorScan: reason === 'ALREADY_SCANNED' && found.scanned_at
            ? {
...
export async function verifyScan(input, scope, ctx = {}) {
    ...
    const outcome = await withTransaction((client) => resolveScan(client, {
...
export async function bulkSync(input, scope, ctx = {}) {
    ...
    const ordered = [...input.scans].sort((a, b) => Date.parse(a.offline_scanned_at) - Date.parse(b.offline_scanned_at));
    const results = [];
    const seen = new Set();
    for (const scan of ordered) {
        ...
        if (seen.has(scan.client_scan_id))
            continue;
        ...
        try {
            ...
            const capturedAt = new Date(Math.min(Date.parse(scan.offline_scanned_at), now));
...
function itemOf(clientScanId, outcome) {
    return {
        clientScanId,
...
function diagnose(row) {
    ...
    if (row.ticket_status === 'REVOKED')
        return 'TICKET_REVOKED';
    if (row.code_status === 'REVOKED')
        return 'CODE_REVOKED';
    if (row.code_kind === 'LOCATION')
        return 'LOCATION_INFO';
    if (row.code_status === 'SCANNED')
...
function messageFor(reason, ticket) {
...
function replayOf(row) {
    const reason = row.result === 'ADMITTED'
        ? 'ADMITTED'
