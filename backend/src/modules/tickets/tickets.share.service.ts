import type { AgentClaims, ShareTicketEmailInput } from '@pravasi/shared';
...
function decodeImage(input: string): Buffer {
  ...
  if (!base64) throw badRequest('Ticket image is malformed');
  ...
  if (!isPng) throw badRequest('Ticket image must be a PNG');

  return buffer;
}

export async function shareTicketByEmail(
  input: ShareTicketEmailInput,
  scope: AgentClaims,
